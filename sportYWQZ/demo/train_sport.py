...
class SportDataset(Dataset):
    def __init__(self, txt, transform=None, target_transform=None):
        ...
        for line in fh:
            line = line.strip('\n')
            words = line.split()
            data.append((float(words[0]) / 1500, float(words[1]) / 700, int(words[2])))
        self.data = data
...
def train():
    ...
    train_data = SportDataset(txt='dataset/sport_train.txt')
    val_data = SportDataset(txt='dataset/sport_val.txt')
    batchsize = 10
